...
class AddAFriendTest < ActionDispatch::IntegrationTest
  ...
  test "that adding a friend works" do
    sign_in_as users(:jure), "testing"

    get "/user_friendships/new?friend_id=#{users(:anze).profile_name}"
    assert_response :success

    assert_difference 'UserFriendship.count', 2 do
      post "/user_friendships", user_friendship: { friend_id: users(:anze).profile_name }
