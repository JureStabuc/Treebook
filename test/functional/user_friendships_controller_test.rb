...
class UserFriendshipsControllerTest < ActionController::TestCase
  context "#index" do
    ...
    context "when logged in" do
      setup do
        @friendship1 = create(:pending_user_friendship, user: users(:jure), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
        @friendship2 = create(:accepted_user_friendship, user: users(:jure), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
  ...
  context "#new" do
    ...
    context "when logged in" do
      ...
      should "display the friend's name" do
        get :new, friend_id: users(:anze)
        assert_match /#{users(:anze).full_name}/, response.body
      end
      ...
      should "assign a new user friendship to the correct friend" do
        get :new, friend_id: users(:anze)
        assert_equal users(:anze), assigns(:user_friendship).friend
      end

      should "assign a new user friendship to the currently logged in user" do
        get :new, friend_id: users(:anze)
        assert_equal users(:jure), assigns(:user_friendship).user
      end
      ...
      should "ask if you really want to friend the user" do
        get :new, friend_id: users(:anze)
        assert_match /Do you really want to friend #{users(:anze).full_name}?/, response.body
      end
    end
  end
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:pending_user_friendship, user: users(:jure), friend: @friend)
        create(:pending_user_friendship, friend: users(:jure), user: @friend)
        sign_in users(:jure)
  ...
  context "#edit" do
    ...
    context "when logged in" do
      setup do
        @user_friendship = create(:pending_user_friendship, user: users(:jure))
        sign_in users(:jure)
  ...
  context "#destroy" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:jure))
        create(:accepted_user_friendship, friend: users(:jure), user: @friend)
